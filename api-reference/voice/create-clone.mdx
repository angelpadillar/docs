...
<ParamField header="x-api-key" type="string" required>
  Your API key with `voice:create` permission
</ParamField>
...
curl -X POST "https://api.buena.ai/api/v2/voice-clones" \
  ...
  -F "settings={\"stability\":0.7,\"clarity\":0.8,\"use_speaker_boost\":true}"
```
...
<ResponseField name="data" type="object">
  ...
  <Expandable title="Voice Clone Object">
    <ResponseField name="id" type="string">
      Unique voice clone identifier (e.g., "voice_abc123")
    </ResponseField>
    ...
    <ResponseField name="settings" type="object">
      ...
      <Expandable title="Voice Settings">
        ...
        <ResponseField name="clarity" type="number">
          Voice clarity/similarity boost (0.0-1.0)
        </ResponseField>
        <ResponseField name="use_speaker_boost" type="boolean">
          Whether speaker boost is enabled
        </ResponseField>
      </Expandable>
    </ResponseField>
    <ResponseField name="training_progress" type="number">
...
| Supported languages       | 29+         | ElevenLabs supported languages      |
...
async function checkTrainingStatus(voiceId) {
  const response = await fetch(
    `https://api.buena.ai/api/v2/voice-clones/${voiceId}`,
    {
      headers: { "x-api-key": "YOUR_API_KEY" },
    }
  );
